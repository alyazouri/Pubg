// سكربت PAC محسن لـ PUBG Mobile
// الهدف: رفع نسبة اللاعبين الأردنيين في اللوبي والمباريات + تقوية التجنيد/البحث
// ملاحظة: لا يوجد DIRECT نهائيًا — كل الترافيك يمشي عبر بروكسي أردني فقط.
...
// عدد الرفض داخل نافذة زمنية لكل فئة
var MATCH_MAX_REJECTS = 10;              // رفض أكثر لسيرفرات الماتش غير الأردنية
var MATCH_WINDOW_MS   = 30 * 1000;

var LOBBY_MAX_REJECTS = 7;               // رفض أكثر لسيرفرات اللوبي غير الأردنية
...
// =================== بادئات IPv6 أردنية (تقريبية) =================== //
// ملاحظة: مطابقة نصية بسيطة على بداية العنوان IPv6.

var JO_V6_PREFIX = {
  LOBBY: [
    "2001:4700::/32", "2c0f:f530::/32", "2c0f:f4a0::/32", "2001:4180::/32",
    "2a00:1a28::/32", "2a02:2e0::/32"
  ],
  MATCH: [
    "2001:4700::/32", "2c0f:f530::/32", "2c0f:f4a0::/32", "2001:4180::/32",
    "2a00:1a28::/32", "2a02:2e0::/32"
...
var JO_V4_RANGES = [
  ...
  // إضافات جديدة لتغطية أوسع
  ["185.76.8.0",     "185.76.11.255"],    // 185.76.8.0/22
  ["185.88.176.0",   "185.88.179.255"],   // 185.88.176.0/22
  ["185.110.52.0",   "185.110.55.255"],   // 185.110.52.0/22
  ["185.140.248.0",  "185.140.251.255"]   // 185.140.248.0/22
];
...
var GEO_TTL_MS          = 60 * 60 * 1000;   // احتياطي لو أضفت GeoIP مستقبلًا
...
function ip6HasPrefix(ip, prefix) {
  // prefix مثل "2001:4700::/32"
  var base = prefix.split("/")[0]; // "2001:4700::"
  base = base.replace("::", ":");
...
// =================== نظام الرفض الذكي للماتش/اللوبي =================== //
...
function FindProxyForURL(url, host) {
  ...
  if (phase === "LOBBY" || phase === "MATCH" || phase === "RECRUIT_SEARCH" ||
    ...
    // لو السيرفر مش أردني في لوبي / ماتش / تجنيد → نحاول نرفضه ضمن نافذة زمنية
    if (!isJo && (phase === "LOBBY" || phase === "MATCH" || phase === "RECRUIT_SEARCH")) {
      if (canRejectPhase(phase === "MATCH" ? "MATCH" : "LOBBY")) {
        // منع الاتصال بهذا السيرفر عشان اللعبة تغيّر IP
        return BLOCK_REPLY;
      }
    }

    // في كل الأحوال PUBG نفسه يطلع عن طريق البروكسي الأردني
    return buildProxyForPhase(phase);
  }

  // 5) باقي الترافيك (غير PUBG)
  // طلبك: لا يوجد DIRECT نهائيًا → كل شيء يمشي عبر البروكسي.
